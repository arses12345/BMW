...
**LOQ Gateway** is a fast, modern, self-hosted **multi-protocol proxy management panel**, built with **Python + FastAPI**, designed to deploy in minutes on **Railway**.
...
> 💡 Originally built around a simple VLESS-over-WebSocket relay, LOQ has evolved into a full multi-protocol gateway with authentication, quota tracking, and a polished management UI.
...
```mermaid
flowchart LR
    A[Client / v2rayNG / NekoBox] -->|VLESS / Trojan / SS| B(LOQ Gateway<br/>FastAPI Server)
    B --> C{Protocol Router}
...
```bash
# Clone your fork
git clone https://github.com/<your-username>/LOQ.git
cd LOQ
...
**LOQ Gateway** یک پنل مدیریت پروکسی چندپروتکلی، سریع و مدرن است که با **Python + FastAPI** ساخته شده و در چند دقیقه روی **Railway** قابل دیپلوی است.
...
```mermaid
flowchart RL
    A[کلاینت / v2rayNG / NekoBox] -->|VLESS / Trojan / SS| B(LOQ Gateway<br/>سرور FastAPI)
    B --> C{مسیریاب پروتکل}
